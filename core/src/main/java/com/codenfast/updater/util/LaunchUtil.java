...
public final class LaunchUtil
{
     ...
     * attempts to relaunch Updater in that directory which will cause it to upgrade to the newly
     * specified version and relaunch the application.
     *
     * @param appdir the directory in which the application is installed.
     * @param updaterJarName the name of the updater jar file in the application directory. This is
     * probably {@code updater-pro.jar} or {@code updater-retro-pro.jar} if you are using
     * the results of the standard build.
     * @param newVersion the new version to which Updater will update when it is executed.
     ...
     * Updater will not cause the application to be upgraded, so the application will have to
     * resort to telling the user that it is in a bad way.
     */
    public static boolean updateVersionAndRelaunch (
            File appdir, String updaterJarName, String newVersion, String javaLocalDir)
        throws IOException
    {
        // create the file that instructs Updater to upgrade
        File vfile = new File(appdir, "version.txt");
        try (PrintStream ps = new PrintStream(new FileOutputStream(vfile))) {
            ps.println(newVersion);
        }

        // make sure that we can find our updater.jar file and can safely launch children
        File pro = new File(appdir, updaterJarName);
        ...
        try {
            ...
            log.warning("Failed to run updater", ioe);
            return false;
        }
    }
    ...
    /**
     * Upgrades Updater by moving an installation managed copy of the Updater jar file over the
     * non-managed copy (which would be used to run Updater itself).
     *
     ...
     */
    public static void upgradeUpdater (File oldgd, File curgd, File newgd)
    {
        // we assume updater's jar file size changes with every upgrade, this is not guaranteed,
        // but in reality it will, and it allows us to avoid pointlessly upgrading updater every
        // time the client is updated which is unnecessarily flirting with danger
        if (!newgd.exists() || newgd.length() == curgd.length()) {
            return;
        }

        log.info("Updating Updater with " + newgd + "...");

        // clear out any old updater
        if (oldgd.exists()) {
        ...
        if (!curgd.exists() || curgd.renameTo(oldgd)) {
            if (newgd.renameTo(curgd)) {
                ...
                try {
                    ...
                    log.warning("Error copying updated Updater back: " + e);
                }
                return;
            }
