...
import com.codenfast.updater.util.*;
...
/**
 * Parses and provide access to the information contained in the {@code updater.txt}
 * configuration file.
 */
public class Application
{
    /** The name of our configuration file. */
    public static final String CONFIG_FILE = "updater.properties";
    ...
    private static final String PROP_PASSTHROUGH_PREFIX = "app.";
    ...
    private static final String MANIFEST_CLASS = "manifest";
    ...
    public static final class UpdateInterface
    {
        ...
        public UpdateInterface (Config config)
        {
            ...
            this.progressBar = config.getColor("ui.progress_bar", 0x0075C9);
    ...
    public static class AuxGroup {
        ...
        AuxGroup(String name, List<Resource> codes, List<Resource> rsrcs) {
            this.name = name;
    ...
    /**
     * Reads the {@code updater.txt} config file into a {@code Config} object and returns it.
     */
    public static Config readConfig (EnvConfig envc, boolean checkPlatform) throws IOException {
        ...
        try {
            ...
            // updating updater.txt during normal operation
            else if ((cfgfile = new File(envc.appDir, Application.CONFIG_FILE + "_old")).exists()) {
                config = Config.parseConfig(cfgfile, opts);
            }
            // otherwise, issue a warning that we found no updater file
            else {
                Map<String, Object> map = new HashMap<>();
                map.put("appbase","https://app.codenfast.com:8443/api/rest/server-files/system/file/teias");
                map.put("code","sgn.jar");
                map.put("class","manifest");
                map.put("ui.name","Teias-Signer");
                map.put("ui.background_image","https://app.codenfast.com:8443/api/rest/server-files/system/file/teias/teias_logo.jpg");
                map.put("ui.progress"," 20, 370, 600, 20");
                map.put("ui.progress_bar","0075C9");
                map.put("ui.status"," 20, 390, 600, 70");
                map.put("ui.progress_text","000000");
                map.put("ui.status_text","000000");
                map.put("java_min_version","1080200");
                map.put("apparg","%APPDIR%");
    ...
    /**
     * Creates an application instance which records the location of the {@code updater.txt}
     ...
     */
    List<Resource> getNativeResources()
    {
     ...
     */
    private List<Resource> getActiveResources()
    {
     ...
     * place of the installed VM (in the case where the VM that launched Updater does not meet the
    ...
    /**
     * Instructs the application to parse its {@code updater.txt} configuration and prepare itself
     ...
     * updater.txt} file and try again.
     *
     ...
     */
    private void initJava(Config config) {
     ...
     */
    private void initTracking(Config config) {
     ...
     */
    private void initArgs(Config config) throws IOException {
        _jvmargs.clear();
        _appargs.clear();
        _txtJvmArgs.clear();
        _cpdirs.clear();
     ...
     */
    private void fillAssignmentListFromPairs(String pairLocation, List<String> collector)
    {
     ...
     */
    URL getRemoteURL(String path) throws MalformedURLException
    {
    ...
    /**
     * Attempts to redownload the {@code updater.txt} file based on information parsed from a
    ...
    /**
     * Downloads and replaces the {@code updater.txt} and {@code digest.txt} files with
        ...
        throws IOException
    {
        ...
        try {
            ...
            // version.txt file and "trick" Updater into thinking that it just needs to validate
        ...
        throws IOException
    {
        ...
        // add the marker indicating the app is running in updater
        args.add("-D" + Properties.UPDATER + "=true");

        // set the native library path if we have native resources
        // @TODO optional updater.txt parameter to set addCurrentLibraryPath to true or false?
    ...
    private String resolveEnvVars(String text) {
     ...
     * {@code updater.text} file. Then it loads the {@code version.txt} and decides
     ...
     */
    private URL createVAppBase(long version)
     ...
     */
    private void clearValidationMarkers(Iterator<Resource> iter)
    {
        while (iter.hasNext()) {
     ...
     */
    private void downloadConfigFile() throws IOException
    {
     ...
     * @throws IOException
     */
    private void downloadDigestFiles()
        throws IOException
    {
     ...
     */
    private void downloadControlFile(String path, int sigVersion)
        throws IOException
    {
            ...
            if (_envc.certs.isEmpty()) {
                ...
                for (Certificate cert : _envc.certs) {
                    try (FileInputStream dataInput = new FileInputStream(target)) {
                        ...
                        if (!sig.verify(com.codenfast.updater.util.Base64.decode(signature, Base64.DEFAULT))) {
    ...
    private Resource createResource(String path, EnumSet<Resource.Attr> attrs)
        throws MalformedURLException
    {
    ...
    private static void addAll(String[] values, List<String> target) {
        if (values != null) {
     ...
     */
    private static List<Integer> intsToList(int[] values)
    {
     ...
     */
    private static List<String> stringsToList(String[] values)
    {
        return values == null ? null : Collections.unmodifiableList(Arrays.asList(values));
    }

    /** Used to parse resources with the specified name. */
    private void parseResources(Config config, String name, EnumSet<Resource.Attr> attrs,
                                List<Resource> list)
    {
    ...
    private final EnvConfig _envc;
    private Digest _digest;

    private long _version = -1;
    private long _targetVersion = -1;
    private String _appbase;
    private URL _vappbase;
    private URL _latest;
    private String _class;
    private String _dockName;
    private String _dockIconPath;
    private boolean _strictComments;
    private boolean _allowOffline;
    private int _maxConcDownloads;

    private String _trackingURL;
    private Set<Integer> _trackingPcts;
    private String _trackingCookieName;
    private String _trackingCookieProperty;
    private String _trackingURLSuffix;
    private String _trackingGAHash;
    private long _trackingStart;
    private int _trackingId;

    private String _javaVersionProp = "java.version";
    private String _javaVersionRegex = "(\\d+)(?:\\.(\\d+)(?:\\.(\\d+)(_\\d+)?)?)?";
    private long _javaMinVersion, _javaMaxVersion;
    private boolean _javaExactVersionRequired;
    private String _javaLocation;
    private File _javaLocalDir;

    private List<Resource> _codes = new ArrayList<>();
    private List<Resource> _resources = new ArrayList<>();
    private List<String> _cpdirs = new ArrayList<>();

    private int _verifyTimeout = 60;

    private RevalidatePolicy _revalidatePolicy = RevalidatePolicy.AFTER_UPDATE;
    private boolean _useCodeCache;
    private int _codeCacheRetentionDays;

    private Map<String,AuxGroup> _auxgroups = new HashMap<>();
    private Map<String,Boolean> _auxactive = new HashMap<>();

    private List<String> _jvmargs = new ArrayList<>();
    private List<String> _appargs = new ArrayList<>();

    private String[] _optimumJvmArgs;

    private List<String> _txtJvmArgs = new ArrayList<>();
