...
/**
 * This class encapsulates all system properties that are read and processed by Updater. Don't
 * stick a call to {@code System.getProperty} randomly into the code, put it in here and give it an
 * accessor so that it's easy to see all of the secret system property arguments that Updater makes
 * use of.
 */
public final class SysProps
{
    ...
    /** Configures the bootstrap appbase (used in lieu of providing a skeleton updater.txt, and as
    ...
    /** Used to debug Updater's launching of an app. When set, it disables redirection of stdout
    ...
    /** If true, Updater installs the app without ever bringing up a UI (except in the event of an
    ...
    /** Instructs Updater to install/update the app without ever bringing up a UI (except in the
    ...
    /**
     * Instructs Updater to launch the app without updating it, or ever bringing up a UI (except
    ...
    /** If true, Updater does not automatically install updates after downloading them. It waits
      * for the application to call `Updater.install`.
    ...
    /** If true, Updater will not unpack {@code uresource} jars. Usage: {@code -Dno_unpack}. */
    public static boolean noUnpack () {
        return Boolean.getBoolean("no_unpack");
    }

    /** If true, Updater will run the application in the same VM in which Updater is running. If
      * false (the default), Updater will fork a new VM. Note that reusing the same VM prevents
      * Updater from configuring some launch-time-only VM parameters (like -mxN etc.).
      * Usage: {@code -Ddirect}. */
    public static boolean direct () {
        return Boolean.getBoolean("direct");
    }

    /** If true, Updater will always try to connect without proxy settings even it a proxy is set
    ...
    /** Specifies the connection timeout (in seconds) to use when downloading control files from
      * the server. This is chiefly useful when you are running in versionless mode and want Updater
